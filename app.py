...
# [요청 반영] 입력란 라벨 및 숫자 크기를 표 수준으로 키우는 CSS
st.markdown("""
<style>
    /* 테이블 디자인 통일 (헤더 색상 및 정렬) */
    th { background-color: #4A5568 !important; color: white !important; text-align: center !important; font-size: 16px !important; }
    td { text-align: center !important; vertical-align: middle !important; font-size: 16px !important; }
    
    /* 입력 위젯(Number Input) 라벨 및 폰트 크기 증폭 */
...
# [요청 반영] 과거 5개년 실적 입력 섹션 (표 디자인과 통일)
st.markdown('<p class="section-title">[과거 5개년 실적]</p>', unsafe_allow_html=True)
...
# [요청 반영] 2026년 최종 예상실적 확정 (크게 배치)
st.markdown('<p class="section-title">[실적 요약 및 기준치 산정]</p>', unsafe_allow_html=True)
...
if st.button("🚀 통합 성과 분석 실행"):
    ...
    방법별 = [
        ("목표부여(2편차)", 기준치 + 2*표준편차_값 if 지표방향=="상향" else 기준치 - 2*표준편차_값),
        ("목표부여(1편차)", 기준치 + 표준편차_값 if 지표방향=="상향" else 기준치 - 표준편차_값),
