...
st.title("🏛️ 경영평가 KPI 목표설정 및 도전성 통합 도구")

# 2. 사이드바: 입력부 (지표 성격 3가지 선택 반영)
with st.sidebar:
    st.header("📋 지표 기본 설정")
    ...
    # 상향, 하향, 일반 중 선택 기능 [새 요청 반영]
    ind_type = st.selectbox("지표 성격 선택", ["상향지표", "하향지표", "일반(비대상)"])
    is_up = ind_type == "상향지표"
    
    # 도전율 설정 (ipywidgets 대신 Streamlit 슬라이더 사용)
...
# 7대 평가방법 목표치 산출 (목표치는 설정하는 게 아니라 자동 산출됨) [새 요청 반영]
...
# 4. 화면 구성 (도전성 5단계 범주 우측 표시)
col_main, col_legend = st.columns([3, 1])

with col_main:
    st.subheader(f"✅ '{ind_name}' 7대 평가방법 산출 결과")
...
# 5. 최종 결과 리포트 (이미지 91881c 양식 반영)
st.divider()
st.subheader("📅 경영평가 상세 결과표")
goal_hi = m_results["산출 목표치"][0]
goal_lo = challenged_base - (2 * std_val if is_up else -2 * std_val)
if ind_type == "일반(비대상)": est_score = 100.0
