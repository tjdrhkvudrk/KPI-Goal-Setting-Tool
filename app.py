...
ax.plot(years_all_label[:6], Y_full, marker='o', color='#2D3748', linewidth=3, label="실적")
ax.plot(years_all_label[5:], line_challenge, linestyle='--', color='#3182CE', label='도전')
ax.plot(years_all_label[5:], line_maintain, linestyle='--', color='#718096', label='유지')
ax.plot(years_all_label[5:], line_conservative, linestyle='--', color='#D69E2E', label='보수')
ax.legend(prop=font_prop)
st.pyplot(fig)

# 4. 도전적 목표 설정 가이드 (여기서 튕김 및 코드 노출 해결)
st.markdown("---")
st.subheader("🎯 4. 도전적 목표 설정 가이드 (담당자 제언)")

# 드롭다운 선택값을 세션에 저장하여 튕김 방지
if 'f_sel' not in st.session_state: st.session_state.f_sel = 4
if 'c_sel' not in st.session_state: st.session_state.c_sel = 5

col_a, col_b = st.columns(2)
with col_a:
    선택방법 = st.selectbox("🎯 담당자 최종 선택", [r['평가방법'] for r in 결과_데이터], index=st.session_state.f_sel, key="final_box")
    st.session_state.f_sel = [r['평가방법'] for r in 결과_데이터].index(선택방법)
with col_b:
    비교방법 = st.selectbox("⚖️ 비교 대상 (대조군)", [r['평가방법'] for r in 결과_데이터], index=st.session_state.c_sel, key="comp_box")
    st.session_state.c_sel = [r['평가방법'] for r in 결과_데이터].index(비교방법)
...
# 핵심: 이 부분의 unsafe_allow_html=True가 누락되면 코드로 보입니다!
st.markdown(f"""
...
""", unsafe_allow_html=True) # <- 이 옵션이 코드가 아닌 디자인으로 나오게 해주는 마법의 단어입니다!
