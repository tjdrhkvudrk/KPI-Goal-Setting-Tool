...
<style>
    .stTable { width: 100%; }
    .stTable td, .stTable th { text-align: center !important; vertical-align: middle !important; font-size: 14px; }
...
if st.button("🚀 통합 성과 분석 실행"):
    ...
    st.table(df.style.format({
        ...
        "예상실적": "{:.3f}", "예상평점": "{:.2f}", "가중치": "{:.2f}", "예상득점": "{:.3f}"
    }).set_properties(**{'text-align': 'center'}))
    
    # [복원] 도전성 단계 설명 박스
    st.markdown("""
    <div style='background-color: #f1f3f5; padding: 15px; border-radius: 8px; font-size: 13px; color: #495057;'>
        <b>🔍 도전성 단계(zp) 기준:</b> 
        (최고목표 - 과거추세치) / 표준오차 기반 산출 <br>
        • <b>🏆 한계 혁신</b> (150%↑): 과거 추세를 완전히 탈피한 매우 도전적 목표 | 
        • <b>🔥 적극 상향</b> (80~150%): 추세 대비 명확한 개선 의지가 반영된 목표 <br>
        • <b>📈 소극 개선</b> (40~80%): 통계적 오차 범위 내의 완만한 상승 목표 | 
        • <b>⚖️ 현상 유지</b> (0~40%): 과거 추세와 유사한 수준의 목표
    </div>
    """, unsafe_allow_html=True)
    ...
    ax.plot(연도_라벨[:-1], 실적_리스트, marker='o', color='#2c3e50', linewidth=3, label='과거 실적 추이', zorder=3)
    ax.plot(연도_라벨[-2:], 전체_실적[-2:], marker='D', markersize=10, color='#e74c3c', linestyle='--', linewidth=2, label='26년 예상실적', zorder=4)

    colors = ['#1abc9c', '#3498db', '#9b59b6', '#f39c12']
    for i, row in df.iterrows():
        ax.scatter("2026년(예)", row['최고목표'], color=colors[i], s=180, edgecolors='white', linewidth=1.5, label=f"{row['평가방법']} 만점기준 ({row['최고목표']:.2f})", zorder=5)

    ax.axhline(기준치, color='#bdc3c7', linestyle=':', linewidth=2, label=f'기준치 ({기준치:.2f})', zorder=1)
    ax.grid(True, axis='y', alpha=0.2)
    ax.spines['top'].set_visible(False)
    ax.spines['right'].set_visible(False)
    ax.legend(prop=font_prop, loc='center left', bbox_to_anchor=(1.02, 0.5), frameon=False, labelspacing=1.0)
