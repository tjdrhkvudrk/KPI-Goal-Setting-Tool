...
<style>
    ...
    .info-box { background-color: #EBF8FF; border: 1px solid #90CDF4; border-radius: 8px; padding: 12px 16px; margin-top: 10px; font-size: 13.5px; color: #2B6CB0; }
    .warn-box { background-color: #FFFBEB; border: 1px solid #F6AD55; border-radius: 8px; padding: 12px 16px; margin-top: 10px; font-size: 13.5px; color: #7B341E; }
    .empty-cell { background-color: #F7FAFC; border: 1px solid #dee2e6; height: 42px; display: flex; align-items: center; justify-content: center; font-size: 13px; color: #A0AEC0; font-style: italic; }
...
# ── 연도별 실적 입력 (None 허용) ──
# session_state에 각 연도별 "입력 여부" 플래그 관리
YEARS = list(range(2021, 2026))

m_cols = st.columns([5.8, 1.6, 2.2])

raw_inputs = {}   # year -> float or None
with m_cols[0]:
    st.markdown('<div class="main-header bg-past">과거 5개년 실적 (2021~2025)</div>', unsafe_allow_html=True)

    # 안내 문구
    st.markdown(
        '<div style="font-size:12px; color:#718096; text-align:center; margin-bottom:4px;">'
        '값이 없는 연도는 빈칸으로 두세요 (최소 2개년 필요)'
        '</div>',
        unsafe_allow_html=True
    )
    ...
    for i, year in enumerate(YEARS):
        with p_cols[i]:
            ...
            # text_input을 사용해 빈칸(None)을 허용
            raw_str = st.text_input(
                f"p_{year}",
                value="" if year < 2023 else str(round(100.0 + (year - 2021) * 5, 3)),
                key=f"v_{year}",
                label_visibility="collapsed",
                placeholder="미입력"
            )
            if raw_str.strip() == "":
                raw_inputs[year] = None
            else:
                try:
                    raw_inputs[year] = float(raw_str.strip())
                except ValueError:
                    raw_inputs[year] = None
                    st.markdown(
                        f'<div style="font-size:11px; color:#E53E3E;">숫자를 입력하세요</div>',
                        unsafe_allow_html=True
                    )

# ── 유효 데이터 추출 ──
valid_pairs = [(year, val) for year, val in raw_inputs.items() if val is not None]
valid_years = [p[0] for p in valid_pairs]
valid_vals  = [p[1] for p in valid_pairs]
n유효 = len(valid_vals)

# 최소 데이터 체크 (2개년 미만이면 추세 계산 불가)
if n유효 < 2:
    st.markdown(
        '<div class="warn-box">⚠️ 유효한 실적이 2개년 미만입니다. 최소 2개년 이상 입력해야 계산이 가능합니다.</div>',
        unsafe_allow_html=True
    )
    st.stop()
...
# ── 안내 메시지: 유효 데이터 개수 표시 ──
if n유효 < 5:
    missing_years = [y for y in YEARS if raw_inputs[y] is None]
    missing_str = ", ".join(str(y) for y in missing_years)
    st.markdown(
        f'<div class="info-box">ℹ️ <b>{n유효}개년</b> 실적이 입력되었습니다 '
        f'({missing_str}년은 미입력). 통계 계산 시 입력된 {n유효}개년 데이터만 사용합니다.</div>',
        unsafe_allow_html=True
    )

# ── 추세 계산: 유효한 연도만 사용 ──
# X축을 실제 연도 인덱스(0~4) 기준으로 매핑
year_to_idx = {y: i for i, y in enumerate(YEARS)}
X_valid = np.array([year_to_idx[y] for y in valid_years], dtype=float)
Y_valid = np.array(valid_vals, dtype=float)

slope_p, intercept_p = np.polyfit(X_valid, Y_valid, 1)
suggested_2026 = round(slope_p * 5 + intercept_p, 3)  # 인덱스 5 = 2026
...
# ── 미래 추세: 2026 예상값 포함한 전체 유효값으로 재계산 ──
X_full = np.append(X_valid, 5.0)   # 인덱스 5 = 2026
Y_full = np.append(Y_valid, 예상_2026)
...
# ── 통계 계산 (유효 데이터 기반) ──
# 3개년: 유효 데이터 중 최근 3개 (연도 순서대로 정렬)
sorted_pairs = sorted(valid_pairs, key=lambda x: x[0])
recent_3 = [v for _, v in sorted_pairs[-3:]] if n유효 >= 3 else valid_vals
recent_5 = valid_vals  # 전체 유효값

avg3  = round(np.mean(recent_3), 3)
std3  = round(np.std(recent_3), 3)
# CAGR 3개년: 유효 최근값 중 처음과 끝
if len(recent_3) >= 2 and recent_3[0] != 0:
    n_period_3 = len(recent_3) - 1
    cagr3 = round(((recent_3[-1] / recent_3[0]) ** (1 / n_period_3) - 1) * 100, 3)
else:
    cagr3 = 0.0

avg5  = round(np.mean(recent_5), 3)
std5  = round(np.std(recent_5), 3)
if len(recent_5) >= 2 and recent_5[0] != 0:
    n_period_5 = len(recent_5) - 1
    cagr5 = round(((recent_5[-1] / recent_5[0]) ** (1 / n_period_5) - 1) * 100, 3)
else:
    cagr5 = 0.0
...
# 표준편차 기준: 3개년 이상 있으면 3개년, 없으면 전체 유효값
if n유효 >= 5:
    std_for_target = std5
    std_구간 = "5개년"
elif n유효 >= 3:
    ...
    std_for_target = round(np.std(recent_5), 3)
    std_구간 = f"{n유효}개년"

# 실적 분석용 마지막 유효 실적값
last_valid_val  = sorted_pairs[-1][1]
last_valid_year = sorted_pairs[-1][0]

# ── 실적 분석 참고내용 ──
# 레이블: 3개년이 실제로 몇 개년인지 표시
n3_label = min(3, n유효)
start_3_year = sorted_pairs[-n3_label][0] if n유효 >= n3_label else sorted_pairs[0][0]

st.markdown(f"""
<div class="guide-box">
    <span style="font-weight:bold; color:#2D3748;">📑 실적 분석 참고내용</span><br>
    • <b>과거 {n3_label}개년 실적 분석결과 ({start_3_year}~{last_valid_year}):</b>
      평균 {avg3:.3f}, 표준편차 {std3:.3f}, 연평균 증가율 {cagr3:.3f}%<br>
    • <b>과거 {n유효}개년 실적 분석결과 ({sorted_pairs[0][0]}~{last_valid_year}):</b>
      평균 {avg5:.3f}, 표준편차 {std5:.3f}, 연평균 증가율 {cagr5:.3f}%<br>
    • <b>중장기 전망 분석결과 (2026~2029):</b>
      평균 {avg_f:.3f}, 표준편차 {std_f:.3f}, 연평균 증가율 {cagr_f:.3f}%
</div>
""", unsafe_allow_html=True)
...
if 지표방향 == "상향":
    기준치 = round(float(max(avg3, last_valid_val)), 3)
else:
    기준치 = round(float(min(avg3, last_valid_val)), 3)
...
if 지표방향 == "상향":
    if last_valid_val >= avg3:
        기준치_근거 = f"직전 유효 실적({last_valid_year}년 {last_valid_val:.3f})이 최근 {n3_label}개년 평균({avg3:.3f})보다 높아 직전 실적을 기준치로 채택"
    else:
        기준치_근거 = f"최근 {n3_label}개년 평균({avg3:.3f})이 직전 유효 실적({last_valid_year}년 {last_valid_val:.3f})보다 높아 {n3_label}개년 평균을 기준치로 채택"
else:
    if last_valid_val <= avg3:
        기준치_근거 = f"직전 유효 실적({last_valid_year}년 {last_valid_val:.3f})이 최근 {n3_label}개년 평균({avg3:.3f})보다 낮아(개선된) 직전 실적을 기준치로 채택"
    else:
        기준치_근거 = f"최근 {n3_label}개년 평균({avg3:.3f})이 직전 유효 실적({last_valid_year}년 {last_valid_val:.3f})보다 낮아(더 좋아) {n3_label}개년 평균을 기준치로 채택"
...
draft_html = (
    ...
    + f'과거 {n유효}개년 실적 추세를 바탕으로 분석한 결과, 2027~2029년의 연평균 증가율은 <span class="hl">{cagr_f:.3f}%</span>로 전망됩니다. '
