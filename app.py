...
<div class="guide-box">
    ...
    • <b>과거 5개년 실적 분석결과:</b> 평균 {avg5:.3f}, 표준편차 {std5:.3f}, 연평균 증가율 {round(((실적_리스트[-1]/실적_리스트[0])**(1/4)-1)*100, 3):.3f}%
</div>
""", unsafe_allow_html=True)
...
if st.button("🚀 중장기 성과 및 한계점 분석 실행"):
    ...
    # 모든 숫자 컬럼에 소수점 셋째자리 포맷팅 강제 적용
    numeric_cols = ["기준치", "최저목표", "최고목표", "예상실적", "예상평점", "가중치", "예상득점"]
    st.table(df_res.style.format({col: "{:.3f}" for col in numeric_cols}))
