...
<style>
    ...
    .formula-table th, .formula-table td { border: 1px solid #E2E8F0; padding: 8px; }
...
st.title("📊 계량 성과지표 시뮬레이터 (오류 해결 최종본)")
...
with m_cols[0]:
    ...
    for i, year in enumerate(range(2021, 2026)):
        with p_cols[i]:
            ...
            val = st.number_input(f"p_input_{year}", value=round(100.0 + (i*5), 3), step=0.001, format="%.3f", key=f"v_key_{year}")
            실적_리스트.append(val)
...
with m_cols[1]:
    ...
    예상_2026 = st.number_input("curr_2026_input", value=suggested_2026, step=0.001, format="%.3f", key="v_2026_key")
...
# 6. 추세 그래프 및 시각적 안전 구역 (Shadow Zone)
st.markdown("---")
...
# 7. 담당자 제언 및 타당성 분석 카드 복구
st.markdown("---")
st.subheader("🎯 4. 도전적 목표 설정 가이드 (담당자 제언)")

names = [r['평가방법'] for r in 결과_데이터]
선택방법 = st.selectbox("🎯 최종 선택 목표", names, index=4)
...
<div style="background-color: #ffffff; border: 1px solid #e2e8f0; border-radius: 12px; padding: 25px; box-shadow: 0 4px 6px rgba(0,0,0,0.05);">
    ...
    2. 도전성 수준: <b>{sel['도전성 단계']}</b><br>
    3. 종합 의견: 통계적 변동 범위 분석을 통해 목표의 객관적 타당성을 확보함.
</div>
""", unsafe_allow_html=True)
