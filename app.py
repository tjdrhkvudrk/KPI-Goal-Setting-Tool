...
stats_df = pd.DataFrame({
    ...
    "과거 3개년 평균": [평균_3년], "과거 3개년 표준편차": [표준편차_3년], "당해년도 기준치": [기준치]
})
...
if st.button("🚀 통합 성과 분석 실행"):
    st.success("데이터 분석 및 시뮬레이션이 완료되었습니다.")
    ...
    for 명칭, 최고, 색상 in 방법별:
        ...
        결과_데이터.append({
            ...
            "예상평점": 평점, "가중치": 가중치_값, "예상득점": 득점, "도전성 단계": 단계, "색상": 색상
        })
    ...
    연도_축 = ["'21", "'22", "'23", "'24", "'25", "'26(예)"]
    
    ax.plot(연도_축[:-1], 실적_리스트[:-1], marker='o', color='#2c3e50', linewidth=2.5, label='과거 실적 추이')
    ax.plot(연도_축[-2:], 실적_리스트[-2:], marker='D', markersize=8, color='#e74c3c', linestyle='--', label='2026년 예상실적')
    
    for i, row in df_res.iterrows():
        ax.scatter(연도_축[-1], row['최고목표'], color=row['색상'], s=150, edgecolors='white', zorder=5, 
    ...
    # 4. 논리적 설정 근거 제안 (최하단 배치)
    st.markdown('<div class="logic-box">', unsafe_allow_html=True)
    st.subheader("💡 목표 설정 논리적 근거 제안")
    
    # 가장 높은(상향) 혹은 가장 낮은(하향) 목표를 도전적 목표로 선정
    target_idx = df_res['최고목표'].idxmax() if 지표방향=="상향" else df_res['최고목표'].idxmin()
    best = df_res.loc[target_idx]
    
    st.write(f"본 지표는 성과관리의 객관성을 확보하기 위해 과거 실적의 변동성을 고려한 **{best['평가방법']}** 기준을 적용하였습니다.")
    st.write(f"특히, 과거 3개년 평균과 직전년도 실적 중 높은 수치인 **{기준치:.3f}**를 기준치로 삼고, 이에 통계적 편차를 적용한 **{best['최고목표']:.3f}**를 최종 목표로 설정함으로써, 안정적 목표 설정을 지양하고 **{best['도전성 단계']}** 수준의 도전성을 확보하였습니다.")
